...
void FDC2214_Write16(uint8_t Slve_Addr, uint8_t reg, uint16_t data)
{
    ...
    IIC_Send_Byte_fdc2214(Slve_Addr << 1);
    IIC_Wait_Ack_fdc2214();

    IIC_Send_Byte_fdc2214(reg);
    IIC_Wait_Ack_fdc2214();

    dat=(data >> 8);
    IIC_Send_Byte_fdc2214(dat);
    IIC_Wait_Ack_fdc2214();
...
uint16_t FDC2214_Read16(uint8_t Slve_Addr, uint8_t reg)
{
    ...
    IIC_Wait_Ack_fdc2214();
    IIC_Start_fdc2214();
    IIC_Send_Byte_fdc2214((Slve_Addr << 1) | 0x01);
    IIC_Wait_Ack_fdc2214();
    temp = (uint16_t)(IIC_Read_Byte_fdc2214(1) << 8);
    temp |= IIC_Read_Byte_fdc2214(0);
    IIC_Stop_fdc2214();
...
uint8_t FDC2214_Init(void)
{
    ...
    deviceID = FDC2214_Read16(FDC2214_Addr, FDC2214_MANUFACTURER_ID);//读器件ID
      ...
      printf("MANUFACTURER_ID :0x%x",MANUFACTURER_ID);
    if(deviceID == 0x5449)
    {
        FDC2214_Reset();
        ...
        printf("FDC2214 Init Success!!");
        return 1;
    }
    else
    {
        printf("FDC2214 Init Failed!!");
    }
