#ifndef __MPU6050DMP_H__
#define __MPU6050DMP_H__
#include "math.h"
#include  "I2C.h"

//#define use_spi       //这里用来选择用i2c还是spi，mpu9250两种都可以，mpu6050只能用i2c，则需要注释掉这句话。
   ...
   P1DIR |= (CSN );//设置为输出
  	    ...
  	    P1SEL |= SCK + MISO + MOSI;
  	       ...
  	       UCB0CTL1 = UCSSEL_2 + UCSWRST;
  	         UCB0BR0 = 32;   //这里的波特率为256K根据单片机的速度来设定
  	         UCB0CTL1 &= ~UCSWRST;
}
...
unsigned  char SPI_RW(unsigned char Byte)
{
	 while(!(IFG2 & UCB0TXIFG));// USCI_A0 TX buffer ready?
		  UCB0TXBUF = Byte;//Send value
		  while(!(IFG2 & UCB0RXIFG));//Has USCI_A0 RX buffer received data?
		  return(UCB0RXBUF); //Received value

}
#ifdef use_spi
/*********************************************/
/* 函数功能：给mpu9250的寄存器写值（一个字节） */
/* 入口参数：reg   要写的寄存器地址          */
/*           value 给寄存器写的值            */
/* 出口参数：status 状态值                   */
/*********************************************/
...
unsigned char mpu9250_Read_Reg(unsigned char reg)
{
	unsigned char value;
	NCS_L;                 //CSN=0;
  	SPI_RW(reg+0x80);//发送寄存器值(位置),并读取状态值
	value = SPI_RW(0);
	NCS_H;               //CSN=1;
	return value;
}
...

{
  ...
  for (dmpnum=0;dmpnum<42;dmpnum++)
  {
   // 
#ifdef use_spi
    dmpdatas[dmpnum] = mpu9250_Read_Reg(0x74);
#else    
    dmpdatas[dmpnum] = I2C_Read(0x74);
#endif      
  } 
#ifdef use_spi
  mpu9250_Write_Reg(DETECT_CTRL, 0xcc);
#else   
  I2C_Write(DETECT_CTRL , 0xcc);//FIFO_RST | DMP_RST | DMP_EN | FIFO_EN
#endif 
}
...
#else
     I2C_Write(0x6f,dmpmemorydata[datanum]);
#endif      
     lastbank=bank; 
    };
}
...
#else
        I2C_Write(0x6f,dmpcfgupddata[datacounts]);
#endif 
        }
       datacounts++;
  }
...
//此处注释掉是因为asin占用的RAM太大，没办法，换更好的单片机则可以用。
...
void Init_MPU6050(void)
{
        ...
        mpu9250_Write_Reg(SMPLRT_DIV, 0x04);
        ...
        loadfirmware(); 
        loadcfgupd();
       mpu9250_Write_Reg(DMP_CFG_1, 0x03);

        mpu9250_Write_Reg(DMP_CFG_2, 0x00);

        mpu9250_Write_Reg(DETECT_CTRL, 0xcc);
#else
        
        I2C_Write(PWR_MGMT_1, 0x00);
        I2C_Write(SMPLRT_DIV, 0x04);
        I2C_Write(CONFIG, 0x06);
        I2C_Write(GYRO_CONFIG, 0x18);
        I2C_Write(ACCEL_CONFIG, 0x00);
         loadfirmware(); 
        loadcfgupd();
        I2C_Write(DMP_CFG_1 , 0x03);
        I2C_Write(DMP_CFG_2 , 0x00);
        I2C_Write(DETECT_CTRL , 0xcc);
#endif 
}
