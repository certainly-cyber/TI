...
#define SDA_Value_OLED  (P1IN & BIT1)    //读取SDA引脚的电平值，读之前端口方向设为输入!!!!!!

#define SCK_I2C_OUT_OLED DIR_OLED  |=BIT1
#define SCK_I2C_H_OLED   PORT_OLED  |= BIT1   // SCK
#define SCK_I2C_L_OLED   PORT_OLED  &= ~BIT1

#define SDA_DIR_OUT_OLED    DIR_OLED  |= BIT0     //数据端口方向设为输出   SDA
#define SDA_DIR_IN_OLED     DIR_OLED  &= ~BIT0    //数据端口方向设为输入
#define SDA_I2C_H_OLED      PORT_OLED  |= BIT0
#define SDA_I2C_L_OLED      PORT_OLED  &= ~BIT0
...
void decimal_show(u8 x,u8 y,float a,int m,int n);
